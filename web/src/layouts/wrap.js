...
export default class Wrap extends Component {
  render = () => {
    const styles = css`
      ${GlobalStyles}
      body {
        color: ${this.props.lightsOn ? ColorsAdvanced.secondaryLightsOn : ColorsAdvanced.secondary};
    ...
    return (
      <StaticQuery
        ...
        `}
        render={({ sanitySettings, sanityResume, site: { siteMetadata } }) => {
            ...
            >
              ...
              >
                <Link
                  ...
                  `}
                >
                  <div className="mr-3">{personName.first}</div>
                  <div>{personName.last}</div>
                </Link>
                <OutboundLink
                  eventLabel="Contact"
                  href={`mailto:${sanitySettings.email}`}
                  css={css`
                    border-radius: 100%;
                  ...
                  {sanityResume.socialLinks.map((item, index) => {
                    return (
                      <OutboundLink
                        ...
                        alt={item.name}
                        css={css`
                          background-image: url("${item.icon.asset.localFile.childImageSharp.resize
                            .src}");
                        `}
                      >
                        {item.name}
